...
public class Deck {
	...
	public Deck(int NumberofJokers){
		//call constructor to make a standard deck
		this();
		//add the appropriate number of jokers
		for (int n=0; n<NumberofJokers; n++){
			Card joker = new Card(eSuit.JOKER,eRank.JOKER, 53+n);
			this.getCards().add(joker);
		}
		//shuffle so that the joker is ot always the last card in the deck
		ShuffleCards();
	}
